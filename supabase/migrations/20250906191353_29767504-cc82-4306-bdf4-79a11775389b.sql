-- Update user roles enum with new values
DROP TYPE IF EXISTS public.app_role CASCADE;
CREATE TYPE public.app_role AS ENUM ('administrator', 'מנהל מערכת', 'מנהל', 'צופה');

-- Update existing role column to use the new enum
...
UPDATE public.user_roles SET role = 'administrator' WHERE role = 'owner';
...
DECLARE
  result json;
BEGIN
  -- Check if current user has permission (administrator or מנהל מערכত)
  IF NOT EXISTS (
